...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" target="dialog" href="role.do?method=addInput&id=${roleForm.id}"><span>添加</span></a></li>
			<li><a class="edit" href="role.do?method=updateInput&id=${role.id}" target="navTab" warn="请选择一个用户"><span>修改</span></a></li>
		...
		<tbody>
			<c:if test="${!empty pm.datas}">
				<c:forEach items="${pm.datas}" var="role">
					<tr>
			            <td>
			              <input name="selectFlag" type="checkbox" value="${role.id}" />
			            </td>
			            <td>${role.id}</td>
			            <td>${role.name}</td>
			            <td>
							<a title="删除" target="ajaxTodo" class="btnDel" onclick="del('role.do?method=del&id=${role.id }');">删除</a>
							<a title="编辑" target="dialog" class="btnEdit" href="role.do?method=updateInput&id=${role.id}">编辑</a>
						</td>
          			</tr>
				</c:forEach>
			</c:if>
